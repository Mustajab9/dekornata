...
import 'package:dekornata/models/Cart.dart';
...
class CartBloc extends Bloc<CartEvent, CartState> {
  ...
  Stream<CartState> _mapCartStartedToSate() async* {
    yield CartLoading();
    try {
  ...
  Stream<CartState> _mapCartProductAddedToState(
    ...
    if (state is CartLoaded) {
      try {
        yield CartLoaded(
          cart: Cart(
            demoProducts: List.from(state.cart.demoProducts)
              ..add(event.product),
          ),
        );
      } catch (_) {}
